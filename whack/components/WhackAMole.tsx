...
const WhackAMole = () => {
  ...
  const showPopUp = (x: number, y: number) => {
    ...
    }, 800); // Hide pop-up after 1 second
  };
...
=======
  ...
  const hideMole = (index: number) => {
    setMoles((currentMoles) =>
      currentMoles.map((mole, moleIndex) =>
        moleIndex === index ? false : mole
      )
>>>>>>> Stashed changes
    );
  };
  ...
  useEffect(() => {
    ...
    const moleInterval = isPowerUpActive
      ? (Math.random() * (750 - 350) + 350) * 2
      : Math.random() * (750 - 350) + 350;
    const timer = setInterval(showMole, moleInterval);
  ...
  async function putScore(gameScore: Number, timedOut: boolean) {
    ...
    try {
      const response = await fetch(
        `https://boopabug.azurewebsites.net/api/players/update-score/${nickname}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    <div className="w-full h-full flex flex-col ">
      <div className="h-1/2 w-full flex flex-col gap-3">
        <div className="gameUIBox w-40 flex flex-row gap-2 PressStartFont">
          {" "}
          <svg
        ...
        <div
          className={`gameUIBox w-32 flex flex-row gap-2 PressStartFont ${
        ...
        {/*   <div>Energy Drinks Left: {powerUpUsed ? 0 : 1}</div> */}
        <div
          className={`gameUIBox w-24 flex flex-row gap-2 PressStartFont ${
            lifeLost ? "life-lost-animation" : ""
          }`}
        >
          {" "}
          <svg
      ...
      <div className="h-1/2 w-full flex">
        <div className="grid-container self-end">
          {moles.map((mole, index) => (
            <div
              key={index}
              className={`mole-hole ${mole ? "mole" : ""} ${
                index === bombIndex ? "bomb" : ""
              }`}
              onClick={(e) => handleWhack(index, e)}
            ></div>
          ))}
        </div>
      </div>
